# Example: anapy3 ~/opt/gladier-hedm/scripts/run_nf_flow.py -nCPUs=32 -experimentName=mpe_feb21 -numNodes=10 -startLayerNr=1 -endLayerNr=1 -paramFile=ps_ff_template.txt -deployment=polaris-clutch
...
def arg_parse():
	...
	OrigFileName, OverallStartNr, GlobalPositionFirstLayer, LayerThickness, WFImages, nDistances, NrFilesPerDistance, TopDataDirectory
	OrigFileName in the parameter file must be nf/dataset/filestem (just before the underscore)
	experimentName must match data folder name.''', formatter_class=argparse.RawTextHelpFormatter)
